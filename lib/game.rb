class Game
  attr_reader :player_one, :player_cpu, :current_turn
  def initialize
  ...
  def player_setup_cruiser
    ...
    until @player_cruiser_input.all? {|coord| @player_one.board.valid_coordinate?(coord)} && @player_one.board.valid_placement?(@player_one.cruiser, @player_cruiser_input)
  ...
  def player_setup_submarine
    ...
    until @player_submarine_input.all? {|coord| @player_one.board.valid_coordinate?(coord)} && @player_one.board.valid_placement?(@player_one.submarine, @player_submarine_input)
      puts "I'm sorry, that was not a valid position. Please try again."
      puts "Enter the squares for the Submarine (2 spaces):"
      @player_submarine_input = gets.chomp.upcase.split
    end
